...
def calculate_loss(prediction, ground_truth):
    ...
    # Add sparsity penalty if using SAE
    if hasattr(prediction, 'current_sparsity_penalty'):
        sparsity_loss = prediction.current_sparsity_penalty
        loss = regression_loss + sparsity_loss
    else:
