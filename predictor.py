...
class Decoder(nn.Module):
    def __init__(self, use_interaction):
        ...
        self.latent_size = 128  # Size of the bottleneck layer
        
        # Encoder layers
        if use_interaction:
            input_size = 128  # Same as before for interaction mode
            self.plan_input = nn.Linear(3, 128)
            self.state_input = nn.Linear(3, 128)
        else:
            input_size = 3   # Same as before for non-interaction mode
        ...
        # Sparsity parameters
        self.sparsity_target = 0.05  # Target activation rate
        self.sparsity_weight = 0.01  # Weight of sparsity penalty
        
    def kl_divergence(self, p, q):
        """Calculate KL divergence for sparsity penalty"""
    ...
    def forward(self, init_hidden, plan, gate, init_state):
        ...
        # Initialize average activation tracker with correct size (latent_size)
        avg_activations = torch.zeros(self.latent_size, device=init_hidden.device)
        
        for t in range(30):
            if self.use_interaction:
                plan_input = self.plan_input(plan[:, t, :3])
                state_input = self.state_input(state[:, :3])
                current_input = state_input + plan_input * gate
            else:
                current_input = state[:, :3]
            
            # Encode
            latent = self.encoder(current_input)
            
            # Track average activations for sparsity (mean across batch dimension)
            avg_activations += torch.mean(latent, dim=0)
            
            # Decode
            decoded = self.decoder(latent)
            state = decoded + state[:, :3]
            output.append(state)
        
        # Calculate average activations across time steps
        avg_activations = avg_activations / 30
        
        # Calculate sparsity penalty
        sparsity_penalty = self.kl_divergence(self.sparsity_target, 
                                            torch.mean(avg_activations))
        
        # Store the penalty for use in loss calculation
        self.current_sparsity_penalty = sparsity_penalty * self.sparsity_weight
